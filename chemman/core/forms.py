...
EXP_NUMBER_CHOICES = (("exact", _("=")),) + EXPRESSION_CHOICES[2:]
...
def get_chemical(pk):
    ...
    if not pk:
        return None
    return Chemical.objects.select_related().get(pk=pk)
...
class ListGeneratorForm(forms.Form):
    name = forms.CharField(
        label=_("Name for this List"), required=False, max_length=100
    )
...
class HandbookCommentForm(forms.Form):
    title = forms.CharField(
        label=_("Title"),
        required=False,
        widget=forms.TextInput(attrs={"placeholder": _("Title (optional)")}),
    )
    text = forms.CharField(
        label=_("Text"),
        widget=forms.Textarea(attrs={"placeholder": _("Write your comment")}),
    )
...
class NewChapterForm(forms.Form):
    ...
    synopsis = forms.CharField(
        label=_("Synopsis"), widget=forms.Textarea, required=False
    )
...
class NewPackageForm(forms.Form):
    ...
    def __init__(self, chemical, *args, **kw):
        ...
        for stchem in StoredChemical.objects.filter(chemical=chemical):
            if stchem.msds:
                text = "{}, {}".format(
                    stchem.get_quality_display(), stchem.company
                )
                msds_choices.append((stchem.msds.id, text))
        self.fields["old_msds"] = forms.TypedChoiceField(
            label=_("Or choose present"),
            choices=msds_choices,
            coerce=int,
            empty_value=None,
            required=False,
        )
...
class NewNormalPackageForm(forms.Form):
    content = forms.DecimalField(
        label=_("Content"), max_digits=7, decimal_places=2
    )
    unit = forms.ChoiceField(label=_("Unit"), choices=UNIT_CHOICES)
    composition = forms.ChoiceField(
        label=_("Composition"), choices=COMPOSITION_CHOICES
    )
    container_material = forms.ChoiceField(
        label=_("Container Material"), choices=CONTAINER_MATERIAL_CHOICES
    )
    supplier_ident = forms.CharField(
        label=_("Article No."), max_length=20, required=False
    )
    supplier_code = forms.CharField(
        label=_("Code"), max_length=100, required=False
    )
    supplier_batch = forms.CharField(
        label=_("Batch No."), max_length=30, required=False
    )
...
class NewSpecialPackageForm(NewNormalPackageForm):
    brutto_mass = forms.DecimalField(
        label=_("Brutto Mass"), max_digits=7, decimal_places=2
    )
    brutto_mass_unit = forms.ChoiceField(label=_("Unit"), choices=MASS_CHOICES)
...
class ProfileForm(forms.Form):
    ...
    internal_phone = forms.CharField(
        label=_("Internal Phone"), max_length=30, required=False
    )


class ConsumeSpecialForm(forms.Form):
    mass_after = forms.DecimalField(
        label=_("Mass after"), max_digits=8, decimal_places=4
    )
    ...
    group = forms.ModelChoiceField(
        label=_("Group"), queryset=Group.objects.all().order_by("name")
    )
...
class ConsumeNormalForm(forms.Form):
    ...
    removed_quantity_unit = forms.ChoiceField(label=_("Unit"), choices=[])
    group = forms.ModelChoiceField(
        label=_("Group"),
        required=False,
    ...
    task = forms.CharField(
        label=_("Task"), widget=forms.Textarea, required=False
    )
...
class DeliverOrderForm(forms.Form):
    place = forms.ModelChoiceField(queryset=StoragePlace.objects.all())
...
class ToxActionForm(forms.Form):
    ...
    to_date = forms.DateField(label=_("To Date"), input_formats=["%d.%m.%Y"])
...
class InitialDeliveryForm(forms.Form):
    ...
    content = forms.DecimalField(
        max_digits=7, decimal_places=2, label=_("Content")
    )
    unit = forms.ChoiceField(choices=UNIT_CHOICES, label=_("Unit"))
    composition = forms.ChoiceField(
        choices=COMPOSITION_CHOICES, label=_("Composition")
    )
    container_material = forms.ChoiceField(
    ...
    delivered_for = forms.ModelChoiceField(
        queryset=User.objects.filter(is_active=True)
        .exclude(username__icontains="admin")
        .order_by("username"),
        required=False,
        label=_("Delivered for"),
    )
    best_before = forms.DateField(
        required=False,
        label=_("Best before"),
    ...
    msds = forms.FileField(
        ...
        required=False,
        help_text=_(
            "Leave empty, to get the one from " "the choosen chemical."
        ),
    )
    published = forms.DateField(
        required=False,
    ...
    supplier_ident = forms.CharField(
        max_length=20, required=False, label=_("Supplier ID")
    )
    supplier_code = forms.CharField(
        max_length=100, required=False, label=_("Supplier CODE")
    )
    supplier_batch = forms.CharField(
        max_length=30, required=False, label=_("Supplier BATCH")
    )
    brutto_mass = forms.DecimalField(
        max_digits=7, decimal_places=2, required=False, label=_("Brutto mass")
    )
    brutto_mass_unit = forms.ChoiceField(
        choices=MASS_CHOICES, required=False, label=_("Unit")
    )
    ...
    def __init__(self, *args, **kw):
        ...
        f["place"].widget.attrs.update({"class": "uk-form-small place"})
...
class SearchPackageForm(forms.Form):
    chemical = forms.TypedChoiceField(
        label=_("Chemical"), coerce=get_chemical, choices=_get_search_choices
    )
    storage = forms.ModelChoiceField(
