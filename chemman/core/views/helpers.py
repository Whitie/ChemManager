...
def search_chemical_by_name(search):
    query = (
        ...
        | Q(iupac_name__icontains=search)
        | Q(iupac_name_en__icontains=search)
    ...
    for chem in Chemical.objects.filter(
        Q(synonyms__name__icontains=search), active=True
    ).order_by("name"):
        if chem not in chems:
...
def search_package(data):
    query = Q(stored_chemical__chemical=data["chemical"]) & Q(
        stored_chemical__chemical__active=True
    )
    ...
    if data["content"]:
        ...
        readable.append(
            _("Content {exp} {value} {unit}").format(
    ...
    if data["container_material"]:
        ...
        readable.append(
            _("Container Material = {}").format(data["container_material"])
        )
    q = (
        StoredPackage.objects.select_related()
        .filter(query)
        .order_by("place__storage__department__name", "place__storage__name")
    )
    return q, readable
...
def check_for_old_packages(package, storage):
    return StoredPackage.objects.filter(
        stored_chemical=package.stored_chemical,
        place__storage=storage,
        empty=False,
...
def handle_consumption(req, storage, package):
    ...
    if len(old) > 1:
        req.session["old_packages"] = [x.pk for x in old]
        req.session["storage_id"] = storage.id
        return redirect("core:package-remove")
    elif len(old) == 1:
        old_package = StoredPackage.objects.select_related().get(pk=old[0].pk)
        data = {"reason": gettext("Automatic removal"), "task": "-"}
...
def check_removal(package, inv, remove, remove_unit):
    ...
    if isinstance(to_remove, units.Volume):
        tmp = units.volume_to_mass(
            to_remove.value, to_remove.unit, package.stored_chemical.chemical
        )
        to_remove = units.make_unit(tmp, "g")
    if isinstance(current, units.Volume):
        tmp = units.volume_to_mass(
            current.value, current.unit, package.stored_chemical.chemical
        )
...
def _save_to_db(diff, data, user):
    ...
    if usage > threshold:
        ...
        PackageUsage.objects.create(
            package=data["package"],
            removed_quantity=diff.value,
            removed_quantity_unit=diff.unit,
            used_by=user,
            user=user,
            task=_("Inventory balancing"),
            is_inventory=True,
        )
        return True
    return False
...
def _calculate_difference(data):
    ...
    if abs(diff) > inv:
        msg = gettext(
            "Inventory difference greater than saved stock: " "{} > {}"
        ).format(abs(diff), inv)
        raise ValueError(msg)
    return diff


def save_inventory(data: dict, user: User) -> tuple[bool, str, str | None]:
    try:
    ...
    for i in range(count):
        packages.append(
            StoredPackage.objects.create(
                stored_chemical=bc.stored_chemical,
                place=place,
                content=bc.content,
                unit=bc.unit,
                supplier_ident=bc.ident,
                supplier_code=bc.code,
                stored_by=user,
            )
        )
    return packages
...
def get_usage(data):
    ...
    return (
        PackageUsage.objects.select_related()
        .filter(query)
        .order_by("-usage_date")
    )
    ...
    for chem in query.exclude(id=chemical.id).distinct():
        ...
        if sc:
            ...
            if not restriction:
                ...
                text = "{sc} -> {restr}".format(
                    sc=sc.value, restr=restriction.text
                )
...
def get_inventory_for_storage(chemical, storage):
    ...
    query = dict(
        chemical=chemical,
        packages__place__storage=storage,
...
def get_stocklimit(chemical, storage, type="min", unit="g"):
    limit = StockLimit.objects.filter(
        chemical=chemical, storage=storage, type=type
    ).first()
...
def merge_packages(fill, remove, user) -> units.Mass | units.Volume:
    data = {
    ...
    for p in remove:
        ...
        PackageUsage.objects.create(
            ...
            used_by=user,
            user=user,
            task=_("Filled up from {}").format(p.package_id),
        )
    return content
