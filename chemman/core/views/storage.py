...
def package_info(req: HttpRequest, pid: int):
    package = StoredPackage.objects.select_related().get(pk=pid)
...
def package_info_by_uid(req: HttpRequest, package_id: str):
    pk = int(package_id.split("-")[-1])
    return package_info(req, pk)


def chem_inventory(req: HttpRequest, chem_id: int):
    # Inventory for one chemical (all storage places)
    chem = Chemical.objects.select_related().get(pk=chem_id)
    packages = (
        StoredPackage.objects.select_related()
        .filter(stored_chemical__id__in=chem.storage.all(), empty=False)
    ...
    ctx = dict(
        title=_("Storage of {}").format(chem.name),
        chem=chem,
        packages=packages,
...
def storage_inventory(req: HttpRequest, storage_id: int):
    ...
    for place in StoragePlace.objects.filter(storage=storage):
        for package in StoredPackage.objects.filter(place=place, empty=False):
            pids.append(package.pk)
    req.session["new_packages"] = pids
    ctx = dict(title=_("Storage {}").format(storage.name), storage=storage)
    return render(req, "core/storage/storage.html", ctx)


def storage_index(req: HttpRequest):
...
def storage_classes_info(req: HttpRequest):
    storage_classes = []
    for sc in StorageClass.objects.all().order_by("id"):
        ...
        sc.spec = {"query": {"storage_class__id": sc.pk}, "and": [], "or": []}
...
def info_stored_chemicals(req: HttpRequest):
    chems = []
    ids = []
    for chem in Chemical.objects.select_related().all().order_by("name"):
        if chem.has_inventory:
            chems.append(chem)
            ids.append(str(chem.pk))
    ctx = dict(
        title=_("Stored Chemicals"),
        chems=chems,
        ids=ids,
...
def dispose(req, package_id):
    ...
    if req.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                req, _("{} where marked as disposed").format(stock)
            )
            return redirect("core:package-info", pid=package.pk)
    else:
        form = DisposeForm()
    chem = package.stored_chemical.chemical
    ctx = dict(
        title=_("DISPOSE"), package=package, chem=chem, form=form, stock=stock
    )
        ...
        if new_place:
            ...
            if helpers.can_store_here(package.chemical, place):
                ...
                messages.error(
                    req,
                    _(
                        "Chemical {} can not be stored in place {}. "
                        "Place/Storage is not lockable!"
                    ).format(package.chemical, place),
                )
...
def merge_packages(req, storage_id, chem_id):
    ...
    packages = StoredPackage.objects.filter(
        place__storage=storage, stored_chemical__chemical=chem, empty=False
    )
    if packages.count() <= 1:
        messages.error(req, _("Not enough packages to merge!"))
        return redirect("core:storage-inventory", storage_id=storage.pk)
    ctx = dict(
        title=_("Merge packages"),
        chem=chem,
        packages=packages,
        storage=storage,
    )
...
def store_new_package_2(req, stored_chem_id, storage_id):
    stored = StoredChemical.objects.select_related().get(pk=stored_chem_id)
    storage = Storage.objects.select_related().get(pk=storage_id)
    if req.method == "POST":
...
def consume_special(req, package, inv, discharge):
    ...
    if req.method == "POST":
        ...
        if form.is_valid():
            ...
            if helpers.check_removal(
                package,
                inv,
                usage.removed_quantity,
                usage.removed_quantity_unit,
            ):
                messages.success(
                    req,
                    _("Consume of {mass} was saved.").format(
                        mass=usage.removed_mass
                    ),
                )
                if package.empty:
                    messages.info(req, _("Package was marked as empty."))
                return redirect("core:package-info", pid=package.id)
            else:
                messages.error(
                    req,
                    _(
                        "Consume of {mass} is higher as package inventory "
                        "{inv}."
                    ).format(mass=usage.removed_mass, inv=inv),
                )
                usage.delete()
    else:
        form = ConsumeSpecialForm(initial=initial)
    ctx = dict(
        title=_("Toxic Consume"),
        package=package,
        form=form,
...
def consume(req, package_id):
    ...
    try:
        ...
        messages.error(
            req,
            _("Package with ID {pid} does not exist!").format(
...
def make_inventory(req, storage_id):
    ...
    if req.method == "POST":
        try:
            ...
            return redirect(
                "core:storage-inventory-result", storage_id=storage_id
            )
        except ValueError as err:
            messages.error(req, str(err))
    ctx = dict(
        title=_("Make Inventory"),
        storage=storage,
...
def transfer_many(req):
    ...
    ctx = dict(
        title=_("Transfer Many Packages"),
        storages=storages,
        places=places,
        packages=packages,
    )
...
def api_storages(req, chem_id):
    special_log = Chemical.objects.values_list("special_log", flat=True).get(
        pk=chem_id
    )
    storages = {}
    ...
    for s in _storages.order_by("department", "name"):
        ...
        if special_log and s.type != "through":
            ...
            places = s.places.all()
        for p in places:
            storages[str(s)].append({"id": p.id, "name": p.name})
...
def api_consume_chem(req):
    ...
    tmp = []
    for c in (
        StoredChemical.objects.select_related()
        .filter(query)
...
def api_storage_for_chemical(req):
    ...
    places = (
        StoragePlace.objects.select_related()
        .filter(packages__stored_chemical=stchem)
        .distinct()
    )
    ...
    try:
        ...
        info_to = [
            ...
            for x in User.objects.filter(
                username__in=settings.INFO_WRONG_BRUTTO
            )
            if x.email
        ]
        mail_msg = _(
