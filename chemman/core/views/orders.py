...
from ..forms import DeliverOrderForm, DeliverJSONForm, InitialDeliveryForm
...
def delivery(req):
    ...
    orders = Order.objects.filter(complete=False, sent__isnull=False).order_by(
        "-sent"
    )
    ctx = dict(
        title=_("Delivery"),
...
def delivery_store_packages(req):
    ...
    if get_form.is_valid():
        ...
        packages = helpers.store_packages(
            cd["delivered"], cd["order"], cd["place"], req.user
        )
    else:
    ...
    ctx = dict(
        ...
        ref=packages[0],
        comp=COMPOSITION_CHOICES,
        cont=CONTAINER_MATERIAL_CHOICES,
        mass=MASS_CHOICES,
    )
...
def initial_delivery(req):
    ...
    DeliveryFormSet = formset_factory(
        InitialDeliveryForm, extra=num, max_num=MAX_DELIVERY_FORMS
    )
    if req.method == "POST":
    ...
    try:
        ...
        pass
    ctx = dict(
        title=_("Initial Delivery"),
        num=num,
        formset=formset,
        max_forms=MAX_DELIVERY_FORMS,
    )
