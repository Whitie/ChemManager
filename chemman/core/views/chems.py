...
def index(req):
    ...
    if settings.LINK_FIRST_THROUGH_STORAGE:
        storage = (
            Storage.objects.select_related().filter(type="through").first()
        )
    else:
        storage = None
    ctx = dict(
    ...
    if req.method == "POST":
        ...
        if which == "storage":
            ...
            if form.is_valid():
                ...
                ctx = dict(
                    title=_("Package Search"),
                    packages=packages,
                    query=query,
                    chem=form.cleaned_data["chemical"],
                )
...
def info_hp(req):
    ...
    for h in HazardStatement.objects.select_related().all():
        hs[h.fullref] = {
            ...
            "spec": {
                "query": {"hazard_statements__ref": h.ref},
                "and": [],
                "or": [],
            },
        }
    for h in EUHazardStatement.objects.select_related().all():
        euh[h.fullref] = {
            ...
            "count": h.chemicals.filter(active=True).count(),
            "spec": {
                "query": {"eu_hazard_statements__ref": h.ref},
                "and": [],
                "or": [],
            },
        }
    for p in PrecautionaryStatement.objects.select_related().all():
        ps[p.fullref] = {
            ...
            "spec": {
                "query": {"precautionary_statements__ref": p.ref},
                "and": [],
                "or": [],
            },
        }
...
def info_ghs(req):
    ...
    for pic in GHSPictogram.objects.all():
        ghs[pic.short] = {
            ...
            "spec": {
                "query": {"pictograms__ref_num": pic.ref_num},
                "and": [],
                "or": [],
            },
        }
    ...
    if req.method == "POST":
        ...
        if user is not None:
            ...
            return render_json(
                ...
                {
                    "success": False,
                    "msg": gettext("Wrong username and/or password."),
                },
            )
    return render_json(
        req,
        {
            "success": False,
            "msg": gettext("Wrong request method (only POST allowed)."),
        },
    )
...
def add_bookmark(req):
    bm = Bookmark.objects.create(
        user=req.user, url=req.POST["path"], text=req.POST["name"]
    )
    bm.save()
    ...
    for chem in chems:
        ...
        results.append(
            dict(
                ...
                text=text,
                url=reverse("core:detail-by-slug", kwargs={"slug": chem.slug}),
            )
        )
...
def api_get_lists(req):
    ...
    for cache in ListCache.objects.all().order_by("-added")[10:]:
        d = dict(
            name=cache.name,
            url=list_url(cache.json_query, cache.name),
