...
from .models import WorkDepartment, ProtectionPictogram
...
class OIForm(forms.Form):
    extra_info = forms.CharField(
        label=_("Extra Info"), max_length=100, required=False
    )
    ...
    dep_2 = forms.ModelChoiceField(
        label=_("Department 2"),
        queryset=WorkDepartment.objects.all(),
        required=False,
    )
    ...
    conduct_pics = forms.ModelMultipleChoiceField(
        label=_("Pictograms"),
        queryset=ProtectionPictogram.objects.all(),
        required=False,
    )
    green_cross = forms.BooleanField(
        label=_("Show green cross"), required=False
    )
    first_aid = forms.CharField(label=_("First aid"), widget=forms.Textarea)
    ...
    ext_phone = forms.CharField(label=_("External help number"), max_length=20)
    int_phone = forms.CharField(label=_("Internal help number"), max_length=20)
...
class ReleaseForm(forms.Form):
    substitutes = forms.ModelChoiceField(
        label=_("Substitutes"),
        queryset=OperatingInstruction.objects.all(),
        required=False,
        empty_label=_("Add new"),
    )
